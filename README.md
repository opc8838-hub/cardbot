...
2026-09-12 更新：多语言问候 → 小字拼成 CARD 的功能页 → 左侧导航的企业工作台。支持中英文、黑白主题、文字地球与世界时钟、任务与邮件依据、人工审核、一键播放七步业务演练，以及可由每个演示账号单独选择的扑克牌 Bot。点击 logo 右侧的 Bot 可选择 16 种表情和 12 种颜色；Bot 对话框可拖动、缩小与关闭，按钮跟随 Bot 颜色。顶部手机远程入口可打开扫码与 WeChat/Lark/WhatsApp Channels 的界面预览。全部回答与远程连接均为本地交互，不调用真实知识库、模型、小满或跨设备会话服务。
...
8. 首次点击 logo 右侧的扑克牌 Bot 选择表情和颜色；保存后再次点击即可打开知识助手演示。Bot 配置按演示账号分别保存在当前浏览器，聊天不会上传。
