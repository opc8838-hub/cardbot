...
try {
  ...
  store.users.push({
    ...
    email: "other-manager@cardbot.com",
    password: "cardbot123",
    role: "manager",
    teamId: "other-team",
    avatar: "OT",
  ...
  const shirleyToken = await login("shirley@cardbot.com");
  const miaToken = await login("mia@cardbot.com");
  const alexToken = await login("alex@cardbot.com");
  const otherManagerToken = await login("other-manager@cardbot.com");
