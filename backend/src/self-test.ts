...
process.env.PROSPECT_COVERAGE_MASTER_SECRET ||=
  "cardbot-self-test-coverage-secret-at-least-32-characters";
...
try {
  const salesToken = await login("shirley@cardbot.com");
  const miaToken = await login("mia@cardbot.com");
  const managerToken = await login("alex@cardbot.com");
  const adminToken = await login("admin@cardbot.com");
  const superAdminToken = await login("super@cardbot.com");
  ...
  const oversizedTradeDocument = await request("/api/trade-documents", {
    ...
    body: JSON.stringify({
      ...
      seller: "CardBot Trading Test Co., Ltd.",
  ...
  const forbiddenSuper = await request("/api/accounts", {
    method: "POST",
    headers: { authorization: `Bearer ${adminToken}` },
    body: JSON.stringify({ name: "Forbidden Super", email: `forbidden.super.${Date.now()}@cardbot.com`, password: "forbid123", role: "super_admin" })
  });
  ...
  const account = await request("/api/accounts", {
    ...
    body: JSON.stringify({ name: "Auto Sales", email: `auto.${Date.now()}@cardbot.com`, password: "start123", role: "sales", teamId: "europe" })
  });
