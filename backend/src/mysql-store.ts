...
const defaultUrl = "mysql://cardbot:change_me@127.0.0.1:3306/cardbot";
...
async function acquireProductionInstanceLock(
  ...
  const lockName = `cardbot_backend_${processRole}_single_instance`;
  ...
  if (Number(result?.acquired || 0) !== 1) {
    connection.release();
    throw new Error(
      `同一数据库已有 CardBot ${processRole} 实例运行，拒绝重复启动`
    );
  }
  ...
  const timer = setInterval(async () => {
    try {
      ...
      if (Number(check?.owner_id) !== ownerConnectionId
        ...
        console.error("CardBot 单实例数据库锁已丢失，进程即将退出");
        process.exit(1);
      }
    } catch {
      console.error("CardBot 无法验证单实例数据库锁，进程即将退出");
      process.exit(1);
    }
  }, 15000);
  timer.unref();
...
export async function createMysqlStore(
  ...
  console.log(`CardBot database profile: ${profile.profile} (${profile.database})`);
  const prospectExecutionLockName = `cardbot_prospect_exec_${createHash(
    "sha256"
  ).update(databaseUrl).digest("hex").slice(0, 24)}`;
  const prospectCandidateLockName = `cardbot_prospect_candidate_${createHash(
    "sha256"
  ).update(databaseUrl).digest("hex").slice(0, 24)}`;
...
export const CARDBOT_MYSQL_SCHEMA_VERSION = "1.2.1-004";
...
export async function migrateMysqlSchema(input: {
  ...
  const releaseId = String(input.releaseId || process.env.CARDBOT_RELEASE_ID || "manual").slice(0, 100);
  const pool = mysql.createPool({ uri: databaseUrl, connectionLimit: 4, namedPlaceholders: true });
  const lockConnection = await pool.getConnection();
  let lockAcquired = false;
  try {
    const [lockRows] = await lockConnection.query(
      "SELECT GET_LOCK('cardbot_mysql_schema_migration', 60) AS acquired"
    );
    ...
    await pool.query(`CREATE TABLE IF NOT EXISTS cardbot_schema_migrations (
      ...
      UNIQUE KEY uk_cardbot_schema_release(schema_version, release_id)
    ) ENGINE=InnoDB`);
    ...
    await pool.query(
      `INSERT INTO cardbot_schema_migrations
      ...
      [CARDBOT_MYSQL_SCHEMA_VERSION, releaseId, before.tables, after.tables, before.columns, after.columns]
    );
    return {
      ok: true,
      schemaVersion: CARDBOT_MYSQL_SCHEMA_VERSION,
      releaseId,
    ...
    if (lockAcquired) {
      try {
        await lockConnection.query("SELECT RELEASE_LOCK('cardbot_mysql_schema_migration')");
      } catch {
...
async function ensureSchema(pool: mysql.Pool) {
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS todos (
    ...
    owner_id VARCHAR(64) NOT NULL,
	    ...
	    work_status VARCHAR(32) DEFAULT '',
	    source_refs_json JSON,
	    status_events_json JSON,
	    completion_evidence_json JSON,
	    workday_assignments_json JSON,
  ...
  await ensureColumn(pool, "todos", "work_status", "VARCHAR(32) DEFAULT ''");
  await ensureColumn(pool, "todos", "source_refs_json", "JSON");
  await ensureColumn(pool, "todos", "status_events_json", "JSON");
  await ensureColumn(pool, "todos", "completion_evidence_json", "JSON");
  await ensureColumn(pool, "todos", "workday_assignments_json", "JSON");
  await ensureColumn(pool, "todos", "completion_criteria", "TEXT");
  await ensureColumn(pool, "todos", "next_action_text", "TEXT");
  await ensureColumn(pool, "todos", "blocker_text", "TEXT");
  await ensureColumn(pool, "todos", "updated_at", "DATETIME NULL");
...
async function loadTodos(pool: mysql.Pool): Promise<Todo[]> {
  return (await rows<Record<string, any>>(pool, "SELECT * FROM todos ORDER BY done ASC, created_at DESC")).map((row) => ({
    ...
    workStatus: row.work_status || undefined, sourceRefs: parseJsonObject(row.source_refs_json, []), statusEvents: parseJsonObject(row.status_events_json, []), completionEvidence: parseJsonObject(row.completion_evidence_json, []), workdayAssignments: parseJsonObject(row.workday_assignments_json, []), completionCriteria: row.completion_criteria || undefined, nextAction: row.next_action_text || undefined, blocker: row.blocker_text || undefined, updatedAt: row.updated_at instanceof Date ? row.updated_at.toISOString() : row.updated_at || undefined
  }));
}
...
async function persistAll(pool: mysql.Pool, store: CrmStore) {
  ...
  try {
    ...
    await replaceRows(connection, "todos", (store.todos as Todo[]), (item) => [item.id, item.title, item.type, item.priority, item.dueAt, item.ownerId, item.teamId, item.related, item.done, item.status || "pending", item.pinState || "", item.sortOrder || 0, item.impactAmount ?? null, mysqlDate(item.createdAt), item.historyAt ? mysqlDate(item.historyAt) : null, item.customerId || "", item.dealId || "", item.reminderRuleId || "", item.triggerKey || "", item.snoozedFrom || "", item.snoozeReason || "", item.snoozeCount || 0, item.snoozedBy || "", item.completedAt ? mysqlDate(item.completedAt) : null, item.completedBy || "", item.completionResult || "", item.leadId || "", item.prospectCandidateId || "", item.tenantProspectId || "", item.outreachChannel || "", item.touchpointId || "", item.cancelledAt ? mysqlDate(item.cancelledAt) : null, item.cancellationReason || "", item.workStatus || "", JSON.stringify(item.sourceRefs || []), JSON.stringify(item.statusEvents || []), JSON.stringify(item.completionEvidence || []), JSON.stringify(item.workdayAssignments || []), item.completionCriteria || "", item.nextAction || "", item.blocker || "", item.updatedAt ? mysqlDate(item.updatedAt) : null], "(id,title,type,priority,due_at,owner_id,team_id,related,done,status,pin_state,sort_order,impact_amount,created_at,history_at,customer_id,deal_id,reminder_rule_id,trigger_key,snoozed_from,snooze_reason,snooze_count,snoozed_by,completed_at,completed_by,completion_result,lead_id,prospect_candidate_id,tenant_prospect_id,outreach_channel,touchpoint_id,cancelled_at,cancellation_reason,work_status,source_refs_json,status_events_json,completion_evidence_json,workday_assignments_json,completion_criteria,next_action_text,blocker_text,updated_at)");
