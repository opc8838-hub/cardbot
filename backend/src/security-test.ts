...
const TEST_JWT_SECRET = "cardbot-security-test-secret-at-least-32-characters";
...
try {
  ...
  const shirley = await login("shirley@cardbot.com", "cardbot123");
  ...
  const admin = await login("admin@cardbot.com", "cardbot123");
  ...
  if (!docsUi.ok || !docsHtml.includes("CardBot API 调试")
  ...
  const superAdmin = await login("super@cardbot.com", "cardbot123");
  ...
  const manager = await login("alex@cardbot.com", "cardbot123");
