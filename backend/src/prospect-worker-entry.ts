...
async function startProspectWorker() {
  ...
  await service.start();
  console.log("CardBot independent prospect worker started");
  ...
  const shutdown = async (signal: NodeJS.Signals) => {
    ...
    shuttingDown = true;
    console.log(
      `CardBot prospect worker received ${signal}, shutting down`
    );
    try {
      await service.stop();
      await store.close?.();
      ...
      console.error(
        "CardBot prospect worker shutdown failed: "
...
if (process.env.NODE_ENV !== "test") {
  void startProspectWorker().catch((error) => {
    console.error(
      "CardBot prospect worker startup failed: "
      + (error instanceof Error ? error.message : String(error))
    );
    process.exit(1);
  });
