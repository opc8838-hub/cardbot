# CardBot — 原 CRM 参考文档（历史资料）

> 保留既有 CRM 说明，不代表新界面全部已联通。当前入口、完成度、测试结论以根目录 README.md 和 docs/STATUS.md 为准。旧命令可能使用 POSIX 环境变量语法，Windows 请读 docs/RUNBOOK.md。勿照旧文档直接操作生产数据库。

## WhatsApp 集成

CRM 的 `WhatsApp` 页面加载仓库内的独立服务 `whatsapp-plugin/`。该目录从已验收的
`桌面/CardBot/CRM系统对接` 迁入，保留 Baileys/Meta Provider、PGlite/PostgreSQL、
...
CardBot 是一款面向外贸销售团队的网页版客户管理软件，采用前后端分离架构，后端数据本地化存储到 MySQL。产品核心不是“记录客户”，而是让外贸业务员每天知道：该跟谁、跟进到哪一步、哪些客户有风险、哪些商机最可能成交、团队的数据是否可导入导出和沉淀。
...
```bash
CRM_STORE=mysql CRM_SEED_DEVELOPMENT_DATA=false DATABASE_URL="mysql://user:password@127.0.0.1:3306/cardbot" npm run dev
```
...
CardBot 自有核心代码使用 Apache-2.0，允许使用、修改，但必须保留许可证、版权和 NOTICE。Communication 是独立服务，因
运行时包含 GPL-3.0 的 libsignal，使用 GPL-3.0-only；它同样允许商用，但
分发时必须按 GPL 提供对应源代码并保留声明。

## 11.作者与交流
<img width="888" height="1131" alt="717d70ce8d78bc5ae817d0ea035824fe" src="https://github.com/user-attachments/assets/2e6b9c3a-7e15-4f2f-b06f-16bf095ce1e2" />
...
CardBot 现已提供首版工作助手闭环：

- `GET /api/workday`：读取本人或团队当天同一批任务。
- `POST /api/workday/morning`：固定当天早盘任务批次。
- `POST /api/workday/tasks/:id/status`：记录处理、待审核、受阻或带依据完成。
- `POST /api/workday/evening`：复核早盘同一批任务并返回未完成原因。
- `POST /api/email-draft-workflows`：用历史邮件原文事实创建待审核草稿。
- `POST /api/email-evidence/parse`：解析 `.eml` 原文，提取可回指的发件人、主题、正文原句和附件清单。
- `POST /api/email-draft-workflows/:id/review`：人工通过或驳回。
- `POST /api/email-draft-workflows/:id/deliver`：审核后保存本地草稿，或通过统一连接器推送小满。

小满连接器默认使用 `mock`，不会触碰真实账号。待主办方提供测试账号和接口资料后，可通过 `OKKI_CONNECTOR_MODE=official_api` 或 `playwright_rpa` 切换，业务流程不需要重写。
