...
async function loginAsManager(page: import("@playwright/test").Page) {
  await loginWithCredentials(page, "alex@cardbot.com", "cardbot123", "Alex");
}

async function loginAsSales(page: import("@playwright/test").Page) {
  await loginWithCredentials(page, "shirley@cardbot.com", "cardbot123", "Shirley");
}

async function loginAsAdmin(page: import("@playwright/test").Page) {
  await loginWithCredentials(page, "admin@cardbot.com", "cardbot123", "Admin");
}
...
test.describe("CardBot prototype pages", () => {
  let runId: string;
  ...
  test("development email studio checks configuration, drafts and sends", async ({ page }) => {
    ...
    await page.locator("#companyProfileName").fill("CardBot Export Ltd.");
    await page.locator("#companyProfileWebsite").fill("https://cardbot.example.com");
    await page.locator("#companyProfileEmail").fill("sales@cardbot.example.com");
  ...
  test("reminders and import export modules perform real actions", async ({ page }) => {
    ...
    const download = await downloadPromise;
    expect(download.suggestedFilename()).toContain("CardBot客户清单");
  ...
  test("wecom sync and account management are operational", async ({ page }) => {
    ...
    await page.locator("#accountEmailInput").fill(`auto.account.${runId}@cardbot.com`);
    ...
    await page.locator("#logoutButton").click();
    await loginWithCredentials(page, `auto.account.${runId}@cardbot.com`, `newpw${runId}`, accountName);
    await expect(page.locator("#settings tbody")).not.toContainText("Super Admin");
    await page.locator("#logoutButton").click();
    await loginWithCredentials(page, "admin@cardbot.com", "cardbot123", "Admin");
    await openView(page, "settings");
  ...
  test("executive report exports a presentation-ready file", async ({ page }) => {
    ...
    const scopedReports = await page.evaluate(async () => {
      const readReport = async (email: string) => {
        const loginResponse = await fetch("/api/auth/login", {
          ...
          body: JSON.stringify({ email, password: "cardbot123" })
        });
      ...
      return {
        sales: await readReport("shirley@cardbot.com"),
        admin: await readReport("admin@cardbot.com")
      };
    });
    ...
    await page.evaluate(async () => {
      await fetch("/api/auth/login", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ email: "alex@cardbot.com", password: "cardbot123" })
      });
    });
    ...
    const download = await downloadPromise;
    expect(download.suggestedFilename()).toContain("CardBot-CRM");
