...
const loginScene = (extraClass = '') => `<section class="login-screen ${extraClass}"><div class="login-top"><a class="login-wordmark" href="${appBase}">cardbot<span>®</span></a><span>03 / SIGN IN</span></div><div class="login-perspective"><form class="login-card" data-login-form novalidate><div class="login-card-face login-card-back" aria-hidden="true"><span class="login-card-eyes"><i></i><i></i></span></div><div class="login-card-face login-card-front"><div class="login-content"><span class="login-kicker">CARDBOT WORKSPACE</span><h1>Welcome back.</h1><p>Continue to your company workspace.</p><label><span>Account</span><input name="account" autocomplete="username" aria-label="Account"></label><label><span>Password</span><input name="password" type="password" autocomplete="current-password" aria-label="Password"></label><button class="pill primary login-submit" type="submit">Log in <span>↗</span></button><footer><span>One workday. Every conversation.</span><b><i></i> READY</b></footer></div></div></form></div></section>`;

function armLoginRestState(card: HTMLElement) {
  if(reduced.matches){card.classList.add('login-card-rest');return;}
...
async function entrance() {
  ...
  if(reduced.matches){word.textContent='Hello / 你好';await wait(900);if(run===generation)login();return;}
  await wait(1000);
...
function film() {
  ...
  root.innerHTML=`<section class="intro-film-screen" aria-label="CardBot introduction"><video class="intro-film" autoplay muted playsinline preload="auto" src="${appBase}assets/cardbot-cards-intro.mp4"></video><div class="film-chrome"><span>CARDBOT / FIND YOUR CARD</span>${button('Skip ↗','skip')}</div><span class="film-count">02 — CARDBOT</span></section>`;
  ...
  const advance=()=>{
    ...
    armLoginRestState(filmScreen.querySelector<HTMLElement>('.login-card')!);
...
function login() {
  clean();screen='login';document.documentElement.lang='en';
  root.style.setProperty('--login-motion-duration',`${ENTRY_MOTION.loginDurationMs}ms`);
