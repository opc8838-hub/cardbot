...
export function mountWorkbench(root: HTMLElement, onIntro: () => void) {
  ...
  function change(event: Event) {
    ...
    if(target.id==='demo-user'&&step<0){userId=target.value;localStorage.setItem('cardbot_demo_user',userId);view='overview';notice='';render();}
